...
//// XXX Need to make a new/better font module
...
#include "SDL_ttf.h"			// XX Figure a way to remove this dependency
...
//he only thing one will normally need to use is render_text(), and 
...
namespace font 
{
	...
	typedef std::map<CacheKey, KRE::TexturePtr> RenderCache;
	...
	KRE::TexturePtr render_text_uncached(const std::string& text, const KRE::Color& color, int size, const std::string& font_name)
	{
		...
		if(std::find(text.begin(), text.end(), '\n') == text.end()) {
			...
			int ypos = 0;
			for(graphics::surface part : parts) {
		...
		graphics::surface s;
	#endif
		return KRE::DisplayDevice::GetCurrent()->CreateTexture(s);
	}

	KRE::Texture render_text(const std::string& text, const KRE::Color& color, int size, const std::string& font_name)
	{
