...
#define ASSERT_EQ(a,b) if((a) != (b)) do { std::ostringstream _s; _s << __SHORT_FORM_OF_FILE__ << ":" << __LINE__ << " ASSERT EQ FAILED: " << #a << " != " << #b << ": " << (a) << " != " << (b) << "\n"; if(throw_validation_failure_on_assert()) { throw validation_failure_exception(_s.str()); } else if(throw_fatal_error_on_assert()) { throw fatal_assert_failure_exception(_s.str()); } else { SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "%s\n", _s.str()); output_backtrace(); report_assert_msg(_s.str()); ABORT(); } } while(0)
